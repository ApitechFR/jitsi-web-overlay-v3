...
export class InitSchema1755075608127 implements MigrationInterface {
    name = 'InitSchema1755075608127'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`replay\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uid\` uuid NULL, \`file_path\` varchar(255) NULL, \`status\` enum ('started', 'uploading-rsync', 'uploaded-rsync', 'error-uploading-rsync', 'terminated') NULL, \`message\` varchar(255) NULL, \`conference_name\` varchar(255) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`conference_uid\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`register_event\` (\`id\` int NOT NULL AUTO_INCREMENT, \`confname\` varchar(255) NOT NULL, \`eventid\` varchar(255) NOT NULL, \`jwt\` varchar(255) NOT NULL, \`uploadCallbackUrl\` varchar(255) NOT NULL, \`uploadCallbackDomainUrl\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`feedback\` (\`id\` int NOT NULL AUTO_INCREMENT, \`clientID\` int NULL, \`room\` varchar(255) NULL, \`evaluation\` int NOT NULL, \`comment\` text NULL, \`userAgent\` varchar(255) NULL, \`jmmc_id\` varchar(255) NULL, \`ip\` int NULL, \`date\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
