...
export function WebinarInvitation({ roomName }: WebinarInvitationProps) {
    ...
    const handleGenerateInvitation = async () => {
        ...
        try {
            ...
            if (!res || !res.invitationToken) throw new Error('Aucun token invitation reçu');
            setInvitationToken(res.invitationToken);
            // Génère le lien à partager
            setVisitorLink(`${window.location.origin}/webinar/invite/${res.invitationToken}`);
        } catch (e: any) {
            console.error(e);
